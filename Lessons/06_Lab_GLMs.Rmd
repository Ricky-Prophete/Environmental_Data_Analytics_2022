...
print(Ozone.plot)
```
#state hypothesis/// examine data/// T-test
...
> Ozone levels are below 50 ppm using a one sided t test @ a 95% confidence level
...
Plot1 <- ggplot(EPAair,aes(x=Ozone, fill=as.factor(Year)), alpha = 0.5)+
  geom_density()
print(Plot1)
...
PM2.5byOzone <- ggplot(EPAair, aes(x = Ozone, y = PM2.5))+
  geom_point()+
  geom_smooth(method = "lm", se = TRUE)
print(PM2.5byOzone)

testcor <- cor(x = Ozone, y = PM2.5)
...
the_lm6 <- lm(data= EPAair, PM2.5 ~ Ozone + Year + meanLat + mean())
