...
Plot1 <- ggplot(BRA_complete, aes(x = Date, y = Rainfall))+
  geom_line()+
  geom_smooth(method = "lm", se = FALSE)
print(Plot1)

Plot2 <- ggplot(BRA_complete, aes(x = Date, y = Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm", se = FALSE)
print(Plot2)
...
BRA_Rainfall.ts.Decomposed <- stl(BRA_Rainfall.ts, s.window = "periodic")

plot(BRA_Rainfall.ts.Decomposed)

BRA_Temperature.ts.Decomposed <- stl(BRA_Temperature.ts, s.window = "periodic")

plot(BRA_Temperature.ts.Decomposed)
...
BRA_Rainfall.ts.Components <- as.data.frame(BRA_Rainfall.ts.Decomposed$time.series[,1:3])

BRA_Rainfall.ts.Components <- mutate(BRA_Rainfall.ts.Components, Observation = BRA_complete$Rainfall, Date = BRA_complete$Date) 
...
ggplot(BRA_Rainfall.ts.Components)+
  geom_line(aes(x = Date, y = Observation), size = 0.5)+
