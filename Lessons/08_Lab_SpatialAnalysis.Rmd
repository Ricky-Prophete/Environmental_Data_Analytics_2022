...
wind_raw <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c('HUC_12' = 'factor')) %>%
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% 
  group_by(HUC_8) %>%
  summarise(windpower = sum(AvgWindEnergy))

view(wind_raw)

#Join to HUC_8 features

huc8_wind <- merge(x = huc8_sf,
                   y = wind_raw,
                   by.x = "HUC_8",
                   by.y = "HUC_8")
