...
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #%in% helps w/ lists -- looks for value in specific column

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #%in% -- looks for values between 152 and 304 (inclusive) within the daynum column

# Exercise 1: 
# filter NTL.phys.data for the year 1999
# what code do you need to use, based on the class of the variable?

NTL.phys.data.Only_1999 <- filter(NTL.phys.data, year4 %in% c(1999))


# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phys.data.Tuesday_90thru99 <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake"), year4 %in% c(1990:1999))
...
NTL.phys.data.pipe.example <-
  NTL.phys.data %>%
  filter(lakename %in% c("Tuesday Lake"), year4 %in% c(1990:1999)) %>%
  mutate(month = (month(sampledate))) %>%
  filter(month %in% c(7))

#alternate

NTL.phys.data.pipe.example.2 <-
  NTL.phys.data %>%
...
Summary <- NTL.phys.data.pipe.example.2 %>%
  filter(depth == 0) %>%
  filter(!is.na(temperature_C)) %>%
  summarise(the_mean = mean(temperature_C))
